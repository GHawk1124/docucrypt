...
impl GraphRAGSystem {
    pub fn new(path: &str) -> Result<Self> {
        ...
        let embedding_model = SentenceEmbeddingsModel::new(embedding_model_config)
...
impl GraphRAGManager {
    ...
    pub async fn new(
        ...
        for classification in &classification_levels {
            for tag in &tags {
                ...
                let graph_rag = GraphRAGSystem::new(&db_name)?;
    ...
    pub async fn query_by_prompt(
        ...
        if let Some(system) = self.systems.get(&(classification.to_string(), best_tag.to_string()))
        {
